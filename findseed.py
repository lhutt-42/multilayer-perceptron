...
def run_model(
    dataset_path: str, model_path: str, seed: int
    ...
    split(dataset_path=dataset_path, test_size=0.2, out_dir=data_dir)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Find the best seed for the model.")
    ...
    results = []
    with mp.Pool(processes=mp.cpu_count(), initializer=set_logger_level) as pool:
        for seed in seeds:
    ...
    best_seed, best_loss, best_accuracy, best_precision = min(
